...
def check_if_contains_word_counts(df):
    ...
    third_column_tokens = re.split(':|\s', third_column)
    potential_float = third_column_tokens[2]
    # check if it's a float, or another word
...
def diff_results(df_result_a, df_result_b,sort):
    ...
    wordstep_a = 3 if contains_word_counts_a else 2
    wordstep_b = 3 if contains_word_counts_b else 2
    diff_result = list()
    ...
    for index_a, row_a in tqdm(list(df_result_a.iterrows())):
        ...
        for _, row_b in df_result_b.iterrows():
            ...
            if row_a_OOV or row_b_OOV:
                ...
                pass
            elif row_a[HINT_COLUMN] == row_b[HINT_COLUMN] and \
                ...
                s_tokens_a = set(tokens_a)
                s_tokens_b = set(tokens_b)
                if s_tokens_a.issubset(s_tokens_b) or s_tokens_b.issubset(s_tokens_a):
                    has_pair = True
                    order_dict = dict(zip(tokens_a, range(len(tokens_a))))
...
def setCellColor(wb,ws,df,row,col,color,expected):
    ...
    opts = {'bold': content in expected, 'bg_color': color}
...
def form_data_frame(t_avp, t_oov, diffs, *models):
...
if __name__ == "__main__":
    ...
    #args.input1 = "/home/ifajcik/word2vec/evaluation/ebooks_code_names_1it.cnwae"
    #args.input2 = "/home/ifajcik/word2vec/evaluation/ebooks_32it_30neg_6w_code_names.cnwae"
    #args.out_buff = "diff_{}_{}".format(args.input1,args.input2)
    print("Reading tsvs...")
    ...
    df = form_data_frame(t_avp_diff,t_oov_diff,sequence_diff,args.input1,args.input2)
    print("Formatting...")
